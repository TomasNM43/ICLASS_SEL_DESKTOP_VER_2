...
left_default = [36, 37, 38, 39, 40, 41]
right_default = [42, 43, 44, 45, 46, 47]
...
def contouring(thresh, mid, img, end_points, right=False, demo=False):
    """
...
def eye_tracker(img, shape, demo=False):
    mask = np.zeros(img.shape[:2], dtype=np.uint8)
    mask, end_points_left = eye_on_mask(mask, left_default, shape)
    mask, end_points_right = eye_on_mask(mask, right_default, shape)
    ...
    eyeball_pos_left = contouring(thresh[:, 0:mid], mid, img, end_points_left, demo=demo)
    eyeball_pos_right = contouring(thresh[:, mid:], mid, img, end_points_right, True, demo)
    if demo:
        print_eye_pos(img, eyeball_pos_left, eyeball_pos_right)
    if eyeball_pos_left == 0 and eyeball_pos_right == 0:
        return False
    else:
        return True
